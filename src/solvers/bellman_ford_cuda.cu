...
__global__ void setHasChangedToFalse(bool *hasChanged, int n)
{
  int bdim = blockDim.x, gdim = gridDim.x, bid = blockIdx.x, tid = threadIdx.x;
  int i = bdim * bid + tid;
  int skip = bdim * gdim;
...
bool reduceLargeArray(bool *d_input_real, int size, int blockSize)
{
  bool *d_input;
  cudaMalloc((void **)&d_input, size * sizeof(bool));
  cudaMemcpy(d_input, d_input_real, size * sizeof(bool),
  ...
  while (size > blockSize)
  {
    ...
    {
      printf("Kernel launch failed for blockReduceOr: %s\n",
  ...
  // value
  blockReduceOr<<<1, blockSize, sharedMemorySize>>>(d_input, d_intermediate, size);
...
bool reduce(bool *arr, int size)
{
  bool beginning = false;
  for (int i = 0; i < size; i++)
  {
    beginning = beginning | arr[i];
  }
  return beginning;
}
  ...
  for (int iter = 0; iter < size - 1; iter++)
  {
    for (int source = 0; source < (*n); ++source)
    {
      if (wasUpdatedLastIter[source])
      {
        ...
        if (kernelErr != cudaSuccess)
        {
          printf("bellmanFordIteration launch failed in iter %d: %s\n", iter, cudaGetErrorString(kernelErr));
        }
      }
    }
    cudaError_t err = cudaMemcpy(wasUpdatedLastIter, d_hasChanged, size * sizeof(bool),
                                 cudaMemcpyDeviceToHost);
    if (err != cudaSuccess)
    {
    ...
    setHasChangedToFalse<<<gdim, bdim>>>(d_hasChanged, size);
    cudaDeviceSynchronize();
    cudaError_t kernelErr = cudaGetLastError();
    if (kernelErr != cudaSuccess)
    {
      printf("setHasChangedToFalse launch failed in iter %d: %s\n", iter, cudaGetErrorString(kernelErr));
    }
  }

  // check for cycle
  printf("Check for cycle\n");
  for (int source = 0; source < (*n); ++source)
  {
    if (wasUpdatedLastIter[source])
    {
    ...
    bool hasChange = reduceLargeArray(d_hasChanged, size, threadsPerBlock); // reduce(temp, size);
    if (hasChange)
    {
      tend = gettime();
